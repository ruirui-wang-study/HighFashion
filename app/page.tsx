...
export default function HomePage() {
  ...
  return (
    <>
      <Section className="overflow-hidden pt-8 lg:pt-12">
        <Container className="grid items-center gap-8 lg:grid-cols-[1.05fr_.95fr]">
          <div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Button asChild size="lg" variant="lime"><Link href="/collections/support">Shop Support Gear</Link></Button><Button asChild size="lg" variant="outline"><Link href="/shop">Build Your Training Kit</Link></Button><Button asChild size="lg" variant="outline"><Link href="/guides">Explore Guides</Link></Button></div>
          </div>
      ...
      <Section className="bg-graphite text-white"><Container><SectionHeader eyebrow="Guides" title="Buy with fit context" body="Short, practical guides help customers choose the right support level and kit setup." /><div className="grid gap-4 lg:grid-cols-3">{guides.slice(0, 3).map((guide) => <GuideCard key={guide.slug} guide={guide} variant="dark" />)}</div><div className="mt-6"><Button asChild variant="lime"><Link href="/guides">Browse all guides</Link></Button></div></Container></Section>
    </>
  );
}
