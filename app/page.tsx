...
export default function HomePage() {
  return (
    <>
      ...
      <Section><Container><SectionHeader eyebrow="Best sellers" title="High-frequency training essentials" body="A focused kit of support, carry, hydration, and sweat-control products for warm-weather sessions." /><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{products.slice(0, 4).map((product) => <ProductCard key={product.id} product={product} />)}</div></Container></Section>
      ...
      <Section className="bg-white"><Container><SectionHeader eyebrow="Compare" title="Choose by training need" /><div className="overflow-hidden rounded-[1.5rem] border border-graphite/10"><div className="grid grid-cols-4 bg-graphite p-4 text-xs font-bold uppercase tracking-[0.16em] text-white"><span>Product</span><span>Best for</span><span>Support</span><span>Carry</span></div>{products.slice(0, 5).map((product) => <div key={product.id} className="grid grid-cols-4 border-t border-graphite/10 bg-warm p-4 text-sm"><span className="font-bold">{product.title}</span><span>{product.useCases.join(", ")}</span><span>{product.category === "Support" || product.category === "Recovery" ? "High" : "Light"}</span><span>{product.category === "Carry" || product.category === "Hydration" ? "Yes" : "No"}</span></div>)}</div></Container></Section>
