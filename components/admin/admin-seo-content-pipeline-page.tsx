...
export function AdminSeoContentPipelinePageClient() {
  ...
  async function load() {
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : "Failed to load content pipeline");
    }
  }
  ...
  async function publish(id: string) {
    if (!window.confirm("Publish this content item manually?")) return;
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader eyebrow="Growth" title="Content Pipeline" body="Convert keyword opportunities into briefs and drafts, then publish only after manual review." />
      <AdminSeoNav />
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b border-graphite/5 align-top">
                ...
                <td className="px-3 py-4">
                  <Button variant="ghost" onClick={() => void publish(item.id)}>Publish</Button>
                </td>
              </tr>
            ))}
          </tbody>
