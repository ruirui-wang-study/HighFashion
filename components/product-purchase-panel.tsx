...
export function ProductPurchasePanel({ product }: { product: Product }) {
  ...
  const trustItems = [
    { icon: Truck, text: copy.freeShipping },
    { icon: RotateCcw, text: copy.returns },
    { icon: CreditCard, text: copy.secureCheckout },
    { icon: ShieldCheck, text: copy.fitGuarantee },
  ];
  ...
  return (
    <div className="lg:sticky lg:top-24">
      ...
      <div className="mt-4 flex items-center gap-4 text-sm font-bold"><span>{product.rating} / 5 {copy.rating}</span><span className="text-muted">{product.reviewCount} {copy.reviews}</span></div>
      ...
      <div className="mt-6">
        <p className="mb-3 text-xs font-bold uppercase tracking-[0.18em]">{copy.color}: {color}</p>
      ...
      <div className="mt-6">
        <div className="mb-3 flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-[0.18em]">{copy.size}</p><a href="#fit-guide" className="text-xs font-bold uppercase tracking-[0.12em] text-signal">{copy.fitGuide}</a></div>
        <div className="grid grid-cols-4 gap-2">{availableSizes.map(({ size: item, variant }) => {
          ...
          return <button key={item} disabled={out} onClick={() => setSize(item)} className={cn("rounded-2xl border px-3 py-3 text-sm font-black", size === item ? "border-graphite bg-lime" : "border-graphite/15 bg-white", out && "cursor-not-allowed opacity-40")}>{item}<span className="block text-[10px] font-bold normal-case tracking-normal">{out ? copy.out : `${variant.stock} ${copy.left}`}</span></button>;
        })}</div>
      </div>
      ...
      <Button size="lg" variant="lime" className="mt-7 w-full" disabled={!canAdd} onClick={add}>{canAdd ? copy.addToCart : copy.outOfStock}</Button>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-graphite/10 bg-warm/95 p-3 backdrop-blur lg:hidden">
        <div className="mx-auto flex max-w-md items-center gap-3"><div className="flex-1"><p className="text-sm font-bold">{product.title}</p><p className="text-xs text-muted">{color} / {size}</p></div><Button variant="lime" disabled={!canAdd} onClick={add}>{copy.addPrice} {formatCents(priceCents)}</Button></div>
      </div>
    </div>
  );
}
