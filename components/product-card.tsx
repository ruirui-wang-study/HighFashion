    ...
    <article className="group overflow-hidden rounded-[1.75rem] border border-graphite/10 bg-white p-3 transition hover:-translate-y-1 hover:shadow-utility">
      ...
      <div className="p-3">
        ...
        <div className="mt-4 flex items-center justify-between">
          <div><span className="font-bold">{formatPrice(product.price)}</span>{product.compareAtPrice ? <span className="ml-2 text-sm text-muted line-through">{formatPrice(product.compareAtPrice)}</span> : null}</div>
        ...
        <Button className="mt-4 w-full" variant="outline" onClick={() => addItem({ product, color: product.colors[0], size: product.sizes[0] })}>Quick add</Button>
      </div>
    </article>
  );
}
